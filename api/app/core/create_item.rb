class CreateItem
  ...
  Contract(C::KeywordArgs[group_id: C::Num, title: String, user: C::RespondTo[:has_group?], data_provider: C::RespondTo[:create!] ] => ::Dto::Item)
  def self.call(group_id: group_id, title: title, user: user, data_provider: Item)
    check_that_group_id_belongs_to_current_user(user, group_id)
    create_item(group_id, title, data_provider: Item)
  end

  def self.create_item(group_id, title, data_provider: Item)
